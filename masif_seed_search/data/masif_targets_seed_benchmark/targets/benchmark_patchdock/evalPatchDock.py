...
target_pdb_dir = '/home/gainza/lpdi_fs/masif_design_paper/masif/masif_seed_search/data/masif_targets/data_preparation/01-benchmark_pdbs/'
source_pdb_dir = '/home/gainza/lpdi_fs/masif_design_paper/masif/data/masif_helices/data_preparation/01-benchmark_pdbs/'
trans_output_bin = "/home/gainza/lpdi_fs/bin//PatchDock/transOutput.pl"
...
run_dir = 'run_site/'+pdb+'_'+t_chain+'/'
all_scores = []
all_names = []
...
missing_file = []
all_benchmark_files = [x.rstrip() for x in open('../benchmark_lists/merged_receptor_helix_list.txt').readlines()]
missing_files = [x for x in all_benchmark_files if x not in os.listdir(run_dir)]
for outputfile in os.listdir(run_dir):
    if '.' in outputfile:
        continue
    if outputfile == pdb+'_'+s_chain:
...
for ix in range(len(all_scores[:10000])):
    if all_names[ix] == pdb+'_'+s_chain:
        gt_ix.append(ix)

# Translate output
target_pdb_fn = target_pdb_dir+pdb+'_'+t_chain+'.pdb'
source_pdb_fn = source_pdb_dir+pdb+'_'+s_chain+'.pdb'
aligned_pdb_fn = run_dir+pdb+'_'+s_chain+'.{}.pdb'
...
if not ground_truth_present:
    ...
    found = 0
    if len(gt_ix) > 0: 
        transo = Popen([trans_output_bin, pdb+'_'+s_chain, str(1), str(len(gt_ix))], cwd=run_dir, stdout=PIPE, stdin=PIPE)
        transo.communicate()
