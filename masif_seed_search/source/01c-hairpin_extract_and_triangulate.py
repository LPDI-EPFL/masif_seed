...
# Compute all hairpins in the PDB chain.
def computeHairpins(pdbid, pdbfilename, chain_id):
    ...
    # Maximum size of the hairpin between the two helices.
    hairpin_loop_max = 10
    hairpin_max_dist = 10.0
    chain_hairpins = []
    # Once all helices have been computed, see which helix's end is within 'hairpin_loop_max' of the next one
    for helix_ix in range(len(chain_helices) - 1):
        cur_helix = chain_helices[helix_ix]
        next_helix = chain_helices[helix_ix+1]
        if cur_helix[-1][1] + hairpin_loop_max > next_helix[0][1]:
            # Find residues in chain 1 whose CA is within X of a CA in chain 2. 
            first_res1 = None
            last_res2 = None
            for res1 in cur_helix:
                ca1 = chain[res1]['CA']
                within_cutoff = False
                for res2 in next_helix: 
                    ca2 = chain[res2]['CA']
                    if ca1 - ca2 < hairpin_max_dist:
                        within_cutoff = True
                        first_res1 = res1
                        last_res2 = res2
                        break
                if within_cutoff:
                    break
            if first_res1 is not None:
                cur_hairpin = []
                for res in chain: 
                    if res.get_id() == first_res1:
                        cur_hairpin.append(res.get_id())
                    if len(cur_hairpin) > 0:
                        cur_hairpin.append(res.get_id())
                    if res.get_id() == last_res2:
                        break
                chain_hairpins.append(cur_hairpin)

    return chain_hairpins
...
for chain_id in list(chain_ids1):
    helices = computeHairpins(pdb_id, pdb_filename, chain_id)
    if len(helices) == 0:
