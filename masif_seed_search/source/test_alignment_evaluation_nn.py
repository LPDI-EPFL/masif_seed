...
nn_score_atomic = AlignmentEvaluationNN('models/weights_9A_0123.hdf5', selected_features=[0,1,2,3], max_npoints=100) ## Slightly slower but more accurate.
nn_score_atomic.restore_model()

mytest_list = open('lists/testing.txt').readlines()
np.random.shuffle(mytest_list)
mytest_list= mytest_list[:100]
...
for myelem in mytest_list:
    ...
    try:
        feats = np.load('training_data_9A/{}/features.npy'.format(myelem))
    except:
        continue
    feats = np.array(feats)
    labels = np.load('training_data_9A/{}/source_patch_rmsds.npy'.format(myelem))
